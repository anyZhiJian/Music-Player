...
static int exsocket_tcp_init(const char *ip, uint16_t port, exsocket_mode_t mode)
{
    int ret = 0;
...
int exsocket_udp_multicast_drop(int sock, const char* ip)
{
    ...
    multicast_request.imr_interface.s_addr = htonl(INADDR_ANY);
    multicast_request.imr_multiaddr.s_addr = inet_addr(ip);
    if(setsockopt(sock, IPPROTO_IP, IP_DROP_MEMBERSHIP, &multicast_request, sizeof(multicast_request)) < 0)
    {
        return -1;
    }
    return 0;
...
int exsocket_tcp_set_keepalive(int sock)
{
    ...
    int count = 3;    // 探测尝试的次数.如果第1次探测包就收到响应了,则后几次的不再发
...
int exsocket_set_sockaddr(const char *ip, uint16_t port, struct sockaddr_in* sockaddr)
{
    if(!ip || !sockaddr)
    {
        return -1;
...
int exsocket_sendto(int sock, const void *data, unsigned int size, struct sockaddr_in *to)
{
    int to_send = size;
    while(to_send > 0)
    {
        int sent = sendto(sock, data, size, 0, (const struct sockaddr *)to, sizeof(struct sockaddr_in));
        if(sent < 0)
        {
            return -1;
        }
...
int exsocket_recvfrom(int sock, void *data, unsigned int size, struct sockaddr_in *to)
{
