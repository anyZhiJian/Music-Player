...
/**
  * @param iomux_param output mode to see soc/io_mux_reg.h(FUNC_X_*), input mode to see soc/gpio_sig_map.h(*_IN_IDX).
  */
void exgpio_set_iomux(gpio_num_t pin, exgpio_iomux_mode_t exgpio_iomux_mode, int iomux_param)
{
    if(exgpio_iomux_mode == exgpio_iomux_out)
    {
        gpio_iomux_out(pin, iomux_param, false);
    }
    else if(exgpio_iomux_mode == exgpio_iomux_in)
    {
        gpio_iomux_in(pin, iomux_param);
    }
}

/**
  * @note before calling this function, we should initialize the pin mode and pull-up/pull-down settings.
  */
...
/**
  * @param offset according to the order of the gpio_array, the offset of the pins starts from zero.
  */
