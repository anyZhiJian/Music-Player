 ...
 * the previous RMT TX Channel signal will be overridden by the GPIO control signal.
 */
...
rmt_encoder_handle_t exrmt_new_encoder(rmt_bytes_encoder_config_t* bytes_encoder_config, exrmt_encode_t exrmt_encode, void* user_data)
{
    ...
    ESP_ERROR_CHECK(rmt_new_copy_encoder(&copy_encoder_config, &exrmt_encoder->copy_encoder));
    if(bytes_encoder_config != NULL)
        ESP_ERROR_CHECK(rmt_new_bytes_encoder(bytes_encoder_config, &exrmt_encoder->bytes_encoder));
    return &exrmt_encoder->base;
}

void exrmt_rx_register_cbs(rmt_channel_handle_t rx_channel, const rmt_rx_done_callback_t recv_done_cb, void *user_data)
{
    rmt_rx_event_callbacks_t cbs = {
        .on_recv_done = recv_done_cb,
    };
    ESP_ERROR_CHECK(rmt_rx_register_event_callbacks(rx_channel, &cbs, user_data));
}

void exrmt_receive_config(rmt_channel_handle_t rx_channel, void *buffer, size_t buffer_size, uint32_t max_ns, uint32_t min_ns)
{
...
void exrmt_transmit(rmt_channel_handle_t tx_channel, rmt_encoder_handle_t encoder, const void *payload, size_t payload_bytes, int loop_count)
{
    rmt_transmit_config_t transmit_config = {
        .loop_count = loop_count,
    };
    ESP_ERROR_CHECK(rmt_transmit(tx_channel, encoder, payload, payload_bytes, &transmit_config));
}